...
check('显示最优结算方案', await page.locator('text=最优结算方案').first().isVisible());
...
for (const [hash, heading, shot] of routes) {
  await page.goto(`${ENTRY}#/${hash}`, { waitUntil: 'networkidle' });
  const ok = await page
...
console.log('\n[4] 物品补货 → 自动生成账单');
await page.goto(`${ENTRY}#/supplies`, { waitUntil: 'networkidle' });
...
console.log('\n[5] 公约表决 → 自动生效');
await page.goto(`${ENTRY}#/pacts`, { waitUntil: 'networkidle' });
...
console.log('\n[6] 值日打卡与身份切换');
await page.goto(`${ENTRY}#/chores`, { waitUntil: 'networkidle' });
...
await mobile.goto(ENTRY, { waitUntil: 'networkidle' });
